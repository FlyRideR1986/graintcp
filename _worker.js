...
const socksAddress = host => {
  ...
  const v4 = raw.split('.');
  if (v4.length === 4 && v4.every(x => /^\d{1,3}$/.test(x) && Number(x) <= 255)) {
    return new Uint8Array([1, ...v4.map(Number)]);
  }
...
async function socks5Connect(fetcher, targetHost, targetPort, ctx) {
  ...
  try {
    ...
    if (hello[1] === 2) {
      if (username == null || password == null) throw new Error('socks5 auth required');

      const u = enc.encode(username), p = enc.encode(password);
      if (!u.byteLength || u.byteLength > 255 || !p.byteLength || p.byteLength > 255) {
